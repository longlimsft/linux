 ...
 * touch these queues.  That is why this skips the
 * rcu_assign_pointer(driver_data, NULL) + synchronize_rcu() +
 * active_senders drain that mana_hwc_destroy_channel() needs for the
 * runtime teardown race; only the CQ-first ordering below (to fence
 * off a pending interrupt) is required.  Calling this on a live,
 * published channel would be a use-after-free.
 */
