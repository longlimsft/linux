					...
					struct gdma_event *event)
{
	...
	case GDMA_EQE_HWC_INIT_DATA:
		...
		case HWC_INIT_DATA_QUEUE_DEPTH:
			/* Preserve the full 24-bit depth for validation. */
...
int mana_hwc_create_channel(struct gdma_context *gc)
{
	...
	if (q_depth_max > HW_CHANNEL_VF_BOOTSTRAP_QUEUE_DEPTH) {
		/* Setup carries no selected depth; do not rebuild smaller rings
		 * by clamping the device's report.
		 */
		...
		if (!err &&
		    (q_depth_max != hwc->num_inflight_msg ||
		     ...
		     max_resp_msg_size != HW_CHANNEL_MAX_RESPONSE_SIZE)) {
			/* Reject a report different from the allocation inputs. */
			dev_err(hwc->dev,
