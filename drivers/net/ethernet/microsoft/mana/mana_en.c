...
{
	struct gdma_context *gc = ac->gdma_dev->gdma_context;
	return gdma_mana_query_device_cfg(gc, proto_major_ver, proto_minor_ver,
					  proto_micro_ver, max_num_vports);
}
...
EXPORT_SYMBOL_NS(mana_get_gic, "NET_MANA");
...
int mana_create_eq(struct mana_port_context *apc)
{
	...
	for (i = 0; i < apc->num_queues; i++) {
		...
		if (gc->msi_sharing) {
			...
			gic = mana_get_gic(gc, true, apc->port_idx, i, &msi);
			if (!gic)
				goto out;
		...
		apc->eqs[i].eq->eq.irq_map = gic->irq_map;
