...
void mana_ib_remove_cq_cb(struct mana_ib_dev *mdev, struct mana_ib_cq *cq)
{
	...
	/* Atomically exchange: read old value and write NULL in one operation.
	 * Only one thread gets the non-NULL pointer; others get NULL and bail.
	 * This prevents double-free if multiple paths (e.g., QP create error +
	 * CQ destroy) race on the same CQ.  xchg provides a full memory barrier,
	 * which is stronger than the release barrier from rcu_assign_pointer.
	 */
	gdma_cq = xchg(&gc->cq_table[cq->queue.id], NULL);
	if (!gdma_cq)
