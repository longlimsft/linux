...
int mana_ib_create_raw_eq(struct mana_ib_dev *mdev)
{
	...
	mdev->raw_eqs = kcalloc(gc->max_num_queues, sizeof(struct mana_eq),
				GFP_KERNEL);
	if (!mdev->raw_eqs)
		return -ENOMEM;
...
{
	struct mana_ib_adapter_caps *caps = &mdev->adapter_caps;

	caps->max_qp_count = MANA_MAX_NUM_QUEUES;
	caps->max_qp_wr = MAX_SEND_BUFFERS_PER_QUEUE;
	caps->max_mr_count = MANA_IB_MAX_MR;
	caps->max_send_sge_count = MAX_TX_WQE_SGL_ENTRIES;
	caps->max_recv_sge_count = MAX_RX_WQE_SGL_ENTRIES;
}
...
int mana_ib_destroy_adapter(struct mana_ib_dev *dev)
{
	...
	if (dev->adapter_handle == INVALID_MANA_HANDLE)
		return 0;
